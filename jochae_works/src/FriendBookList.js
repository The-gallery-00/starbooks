import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import './FriendBookList.css';

const BookCard = ({ id, title, author, progress, cover, activeTab }) => {
  const navigate = useNavigate();
  const handleCardClick = () => {
    navigate(`/my-bookDetail`, { state: { bookId: id, type: activeTab } });
  };

  return (
    <div className="fbl-book-card-link" onClick={handleCardClick} style={{ cursor: 'pointer' }}>
      <div className="fbl-book-card">
        <img src={cover} alt={title} className="fbl-book-cover" />
        <div className="fbl-book-info">
          <p className="fbl-book-title">{title}</p>
          <p className="fbl-book-author">{author}</p>
          {activeTab !== 'wishlist' && (
            <>
              <div className="fbl-progress-bar-container">
                <div className="fbl-progress-bar" style={{ width: `${progress}` }}></div>
              </div>
              <p className="fbl-progress-text">{progress}</p>
            </>
          )}
        </div>
      </div>
    </div>
  );
};

const FriendBookList = ({ myBooks }) => {
  const [activeTab, setActiveTab] = useState('reading');

  const renderTabContent = () => {
    const books = myBooks[activeTab];
    return (
      <div className="fbl-book-list-container-tab">
        {books && books.length > 0 ? (
          books.map((book, index) => (
            <BookCard key={book.id || index} {...book} activeTab={activeTab} />
          ))
        ) : (
          <p className="no-books">등록된 도서가 없습니다.</p>
        )}
      </div>
    );
  };

  return (
    <section className="library-section">
      <div className="fb-tabs">
        {['reading', 'finished', 'wishlist'].map(tab => (
          <button
            key={tab}
            className={activeTab === tab ? 'active' : ''}
            onClick={() => setActiveTab(tab)}
          >
            {tab === 'reading' ? '읽고 있는 도서' : tab === 'finished' ? '읽은 도서' : '찜한 도서'}
          </button>
        ))}
      </div>
      <div className="tab-content">{renderTabContent()}</div>
    </section>
  );
};

export default FriendBookList;
