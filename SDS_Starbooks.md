...
| 2025-10-29 | 1.1 | 유스케이스 추가 | 프로젝트 팀 |
| 2025-11-4 | 1.2 | 클래스다이어그램 추가 | 프로젝트 팀 |
...
**Friend**
- **속성**: userId(String), nickname(String), requesterId(String), receiverID(String), FriendshipStatus(status:PENDING, ACEPTED, REJECTED)
- **설명**: 시스템 사용자들 간의 친구목록을 관리하는 클래스

**Notification**
- **속성**: notificationID(Long), userID(String), NotificationType(type:SYSTEM, CHALLENGE, FRIEND, COMMENNT 등)
- **설명**: 시스템 사용자 각각에게 알림을 전달하는 클래스

**Announcement**
- **속성**: announcementID(Long), title(String), content(Stirng), createdAt(LocalDateTime), authorID(String)
- **설명**: 시스템 관리자가 사용자들에게 공지를 전달하기 위한 클래스

**Ranking**
- **속성**: rankingId(Long), userID(String), rankPosition(int), rankingType(type:BOOK_READ, PAGES_READ, STREAK_DAYS)
- **설명**: 사용자간의 독서통계를 랭크 매기기 위한 클래스

**Goal**
- **속성**: goalID(Long), userId(String), goalType(String:DAILY, MONTHLY, YEARLY), targetBooks(int), achievedBooks(int), startDate(LocalDate), endDate(LocalDate)
- **설명**: Challenge에서나 시스템 사용자 개인의 목표설정을 위한 클래스

**BookReview**
- **속성**: reviewID(Long), userId(String), bookID(Long), rating(int), reviewContent(String), createdAt(LocalDate), editAt(LocalDate)
- **설명**: 책에 관한 유저들의 리뷰를 위한 클래스 

**PurchaseLink**
- **속성**: id(Long), bookID(Long), stieName(Stirng), url(String)
- **설명**:
